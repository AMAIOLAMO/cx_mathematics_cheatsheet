...
Linear Algebra is the study of adding things together, it studies how the "same type" of things
when added, results in a new thing that is of the "same type".
...
If the vector is in Eucledian space, then we can also say that:
\begin{center}
    \[
        \vec{v} \in \mathbb{R}^n
    \]
...
where:
\begin{itemize}
    \item $\mathbb{R}^n$: is the set of all n-dimensional real vectors.(real as in all values are real numbers)
\end{itemize}
...
\subsubsection{Vectors and Points in Eucledian Space}
...
For a unit quaternion, due to the fact that it's magnitude being 1, it'd inverse
