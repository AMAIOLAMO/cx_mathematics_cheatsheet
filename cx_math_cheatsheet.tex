...
\begin{center}
    \includegraphics[width=10em, height=10em]{linear_algebra_core}
\end{center}
...
For Addition:
\begin{enumerate}
    \item Closed under addition: If $\vec{x}, \vec{y} \in S$ then $\vec{x} + \vec{y} \in S$.
    \item Commutative law: If $\vec{x}, \vec{y} \in S$ then $\vec{x} + \vec{y} = \vec{y} + \vec{x}$.
    \item Associative law: If $\vec{x}, \vec{y}, \vec{z} \in S$ then $(\vec{x} + \vec{y}) + \vec{z} = \vec{x} + (\vec{y} + \vec{z})$.

    \item Existence of an additive identity: $\exists \vec{0} \in S, \forall \vec{v} \in S (\vec{v} + \vec{0} = \vec{v})$.
    \item Exitence of an additive inverse: $\forall \vec{v} \in S (\vec{v} + (-\vec{v}) = \vec{0})$ where $\vec{0}$ is the additive identity.
\end{enumerate}

For Scalar Multiplication, If $a,b \in \mathbb{R}$ and $\vec{x}, \vec{y}, \vec{z}$ is in vector space $S$ then:
\begin{enumerate}
...
\subsubsection{Geometric Representation of Vectors}
...
\begin{center}
    \includegraphics[width=10em, height=10em]{geometric_vector_desc}
\end{center}
...
\subsubsection{The Very Important $\mathbb{R}^n$ Vector Space}
A $\mathbb{R}^n$ vector denotes a list of values, it's mathematical definition is as follows:
...
\subsubsection{$\mathbb{R}^n$ Vector Properties}
...
\subsubsection{$\mathbb{R}^n$ Vector Operations}
...
\subsubsection{$\mathbb{R}^n$ Vectors and Points in Eucledian Space}
...
\subsubsection{Matrix as a Combination of Vectors}
TODO

\subsubsection{Matrix as a Representation of Systems of Linear Equations}
TODO

\subsubsection{Matrix as a Means of Linear Transformation}
TODO: explain Rotation, Scale, Shearing...

\subsubsection{Why is Translation not a form of Linear Transformation}
...
\subsubsection{Matrix for Transforming Between Two Eucledian Spaces}
TODO

\subsubsection{Projection Matricies}
TODO

\subsubsection{Useful Matrix Constructs}
...
\begin{itemize}
    \item $P$: is any point on the plane.
    \item $\hat{n}$: is the unit vector that represents the direction / facing of the plane.
    \item $D$: is a known point on the plane.
\end{itemize}
...
\subsubsection{Quaternions as Rotations for 3D spaces(Versors)}
...
This formula can actually be applied multiple times consecutively to create a resulting rotational displacement
that is the "sum" of all od each individual rotation:
\begin{center}
    \begin{flalign}
        P_q & = b(aPa^{-1})b^{-1} \\
            & = (ba)P(a^{-1}b^{-1}) \\
...
By rearranging, it is obvious that it is equivalent to simply rotating the combination of two
versors $a$ and $b$.
